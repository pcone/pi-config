...
Use `subagent_status` to check progress and the `wait` tool (not `sleep`) to pause for results. `wait(N)` blocks until a subagent completes, the user sends a message, or N seconds elapse — whichever comes first.
