...
- **Consider pre-dispatch simplification.** Before dispatching, evaluate whether the change could be 2× smaller via a small surrounding refactor (missing abstraction, overgrown function/module, special case to generalize, data-shape mismatch — see `agents/review-plan.md` §6). If so, route through `review-plan` first to surface it formally — don't let the implementer fight the existing shape.